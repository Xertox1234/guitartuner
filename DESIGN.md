...
string-lock target. Its **measured-accuracy report** (the spec, from data) lives in
[`docs/benchmarks/accuracy.md`](docs/benchmarks/accuracy.md) and is regenerated by a
headless benchmark on every CI run (synthesized pure/harmonic/**inharmonic** tones across
the full range + SNR sweeps). The strobe `phase` is normalized as a 0…1 cycle position of
the tracked fundamental, so the Aurora strobe scrolls by Δphase for true sub-cent
precision (EXPERIENCE §2).
