...
@Suite struct FrequencyInterpolatorTests {
    let fs = 48_000.0
    let N = 4096
    ...
    @Test func candanIsNearCRLB() {
        ...
        #expect(worst < 0.02, "Candan worst-case \(worst)¢ should be near-CRLB")
    }

    @Test func parabolicAndLogReproduceProbeRanking() {
        let m0 = 200.0
        var worstLin = 0.0, worstLog = 0.0
        for d in stride(from: -0.45, through: 0.45, by: 0.05) {
            let a = hannMag(d, m0: m0, bin: m0 - 1)
            let b = hannMag(d, m0: m0, bin: m0)
            let c = hannMag(d, m0: m0, bin: m0 + 1)
        ...
        #expect((0.40...0.52).contains(worstLin), "parabolic ~0.46¢, got \(worstLin)")
        #expect((0.10...0.18).contains(worstLog), "log-parabolic ~0.14¢, got \(worstLog)")
        #expect(worstLog < worstLin)   // the primary invariant
    }

    @Test func interpolatorsZeroOnCentredPeak() {
        #expect(abs(FrequencyInterpolator.parabolic(0.5, 1.0, 0.5))                      < 1e-12)
        #expect(abs(FrequencyInterpolator.logParabolic(0.5, 1.0, 0.5))                   < 1e-12)
        #expect(abs(FrequencyInterpolator.candan((1, 0), (2, 0), (1, 0), n: N))          < 1e-12)
    }

    @Test func degenerateInputsAreSafe() {
        #expect(FrequencyInterpolator.parabolic(1, 1, 1) == 0)               // flat → no peak
        #expect(FrequencyInterpolator.candan((0, 0), (0, 0), (0, 0), n: N) == 0)  // silence
        #expect(abs(FrequencyInterpolator.parabolic(0, 1, 9)) <= 0.5)        // clamped
    }
}
