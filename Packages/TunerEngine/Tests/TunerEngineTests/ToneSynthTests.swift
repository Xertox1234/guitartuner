...
final class ToneSynthTests: XCTestCase {
    ...
    func testSilentAtZeroGain() {
        var s = ToneSynth(sampleRate: fs, frequency: 440, targetGain: 0)
        let out = render(&s, seconds: 0.05)
        XCTAssertLessThan(out.map { abs($0) }.max() ?? 0, 1e-6)
        XCTAssertEqual(s.currentGain, 0, accuracy: 1e-9)
    }

    func testRampsUpBoundedAndAudible() {
        var s = ToneSynth(sampleRate: fs, frequency: 440, targetGain: 0.8)
        let out = render(&s, seconds: 0.1)
        XCTAssertGreaterThan(s.currentGain, 0.7)                       // reaches near target
        XCTAssertLessThanOrEqual(out.map { abs($0) }.max() ?? 0, 1.0)  // never clips
        XCTAssertGreaterThan(rms(out), 0.05)                          // audible
        XCTAssertFalse(out.contains { $0.isNaN || $0.isInfinite })
    }

    func testRampsDownToSilence() {
        var s = ToneSynth(sampleRate: fs, frequency: 220, targetGain: 1)
        _ = render(&s, seconds: 0.1)
        s.targetGain = 0
        _ = render(&s, seconds: 0.2)
        XCTAssertLessThan(s.currentGain, 1e-3)
    }

    func testHarmonicContentAndDominantFundamental() {
        ...
        XCTAssertGreaterThan(f1, f2)                                   // fundamental dominant
        XCTAssertGreaterThan(f2, 0.001)                               // a real 2nd partial
        XCTAssertGreaterThan(f3, 0.0005)                             // a real 3rd partial
        XCTAssertGreaterThan(f1, between * 5)                         // tonal, not noisy
    }

    func testPlaysRequestedPitch() {
        ...
        XCTAssertGreaterThan(at440, magnitude(out, freq: 415) * 5)
        XCTAssertGreaterThan(at440, magnitude(out, freq: 466) * 5)
    }

    func testPhaseContinuousAcrossRetune() {
        var s = ToneSynth(sampleRate: fs, frequency: 440, targetGain: 1)
        ...
        XCTAssertLessThan(seam, maxStep + 0.05)
    }
}
